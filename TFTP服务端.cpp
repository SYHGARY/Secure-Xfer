...
static int parse_rrq_wrq(const char* buf, int n, char* out_filename, size_t out_sz) {
    ...
    if (mode_end == mode) return 0; // mode 不能为空
...
static SOCKET create_transfer_socket() {
    ...
    addr.sin_port = 0; // system assigns ephemeral port (TID)
...
static int recv_packet(SOCKET sock, char* buf, int bufsz, sockaddr_in* from, int* out_n) {
    ...
    if (n == SOCKET_ERROR) return 0; // timeout or error
    *out_n = n;
    return 1;
}
...
static int build_server_path(const char* filename, char* out, int out_sz) {
    if (!is_safe_filename(filename)) return 0;
    CreateDirectoryA(TFTP_ROOT_DIR, NULL);

    int need = (int)strlen(TFTP_ROOT_DIR) + 1 + (int)strlen(filename) + 1;
    if (need > out_sz) return 0;
    sprintf_s(out, out_sz, "%s\\%s", TFTP_ROOT_DIR, filename);
    return 1;
}
...
static void handle_rrq(const sockaddr_in* client_addr, const char* filename) {
    // server reads from ./tftp_root/filename for download (consistent)
    char path[512];
    if (!build_server_path(filename, path, (int)sizeof(path))) {
    ...
    if (!file) {
        SOCKET tmp = create_transfer_socket();
        if (tmp != INVALID_SOCKET) {
            send_error_packet(tmp, client_addr, ERROR_NOT_FOUND, "File not found");
            closesocket(tmp);
        }
    ...
    long long total = file_size_64(path); // for progress
    ...
    // if total is multiple of 512, must send final zero-byte data block after last full block
    ...
    while (1) {
        ...
        // send and wait ack with retries
        int retries = 0;
        for (;;) {
        ...
        print_progress_bar("[RRQ] sending", sent_bytes, total);

        if (bytes_read < DATA_SIZE) {
            // finished normally (last block <512)
            needs_final_zero = 0;
            break;
        }

        // bytes_read == 512, continue
        block++;
        if (block == 0) block = 1;
    }

    // send final zero-byte data block if needed
    if (needs_final_zero) {
        short op = htons(OP_DATA);
        short bn = htons((short)block);
        memcpy(pkt, &op, 2);
...
static void handle_wrq(const sockaddr_in* client_addr, const char* filename) {
    char save_path[512];
    if (!build_server_path(filename, save_path, (int)sizeof(save_path))) {
        SOCKET tmp = create_transfer_socket();
        if (tmp != INVALID_SOCKET) {
            send_error_packet(tmp, client_addr, ERROR_ACCESS_VIOLATION, "Access violation (bad filename)");
            closesocket(tmp);
    ...
    while (1) {
        ...
        if (!recv_packet(tsock, buf, BUFFER_SIZE, &from, &n)) {
            // timeout: re-ack last good block
        ...
        if (op == OP_DATA) {
            ...
            if (block == expected) {
                ...
                // show progress (unknown total). Print bytes + speed.
                DWORD now = GetTickCount();
                double sec = (now - startTick) / 1000.0;
                double kbps = (sec > 0) ? (received_bytes / 1024.0 / sec) : 0.0;
                printf("\r[WRQ] received %lld bytes (%.1f KB/s) last block=%u   ", received_bytes, kbps, block);
                fflush(stdout);
...
static void start_tftp_server() {
    ...
    printf("Root dir: .\\%s (uploads saved here; downloads read here)\n\n", TFTP_ROOT_DIR);
